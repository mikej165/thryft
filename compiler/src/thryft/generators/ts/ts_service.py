...
class TsService(Service, _TsNamedConstruct):
    def _ts_references_definition(self, caller_file_abspath, caller_stack):
        references = ["""/// <reference path="%s/jquery/jquery.d.ts" />""" % os.path.relpath(os.path.join(self._parent_generator().ts_document_root_dir_path, 'share', 'DefinitelyTyped'), os.path.dirname(caller_file_abspath)).replace(os.path.sep, '/')]
